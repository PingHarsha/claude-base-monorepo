...
5. Run **`/merge`** on the feature branch. It runs the integration test suite (`mvn test -P integration-tests`), shows results + the diff against develop, and **pauses for explicit `approve`**. On approval, it merges to `develop` and deletes the feature branch. (For ad-hoc integration runs not tied to a merge, `/integration-verify` runs the same test suite without the merge step.)
